...
Fig.~\ref{fig:multiseg-demo}, which are commonly seen in the
power/ground network in practical VLSI layouts.

\begin{figure}[!th]
\centering
\includegraphics[width=0.9\columnwidth]{multiseg_demo.eps}
\label{fig:multiseg-demo}
\caption{Electrical current in a multi-segment wire structure (J: current direction)}
\end{figure}
