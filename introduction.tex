...
Fig.~\ref{fig:4seg-demo}, which are commonly seen in the
power/ground network in practical VLSI layouts. As a shown in
Fig.~\ref{fig:4seg-multiplot}, hydrostatic stress is predicted to be able to
noticeably distributed on a certain segment that has almost zero current
density, as well as those segments carrying significant electrical currents.
This shows the distinct benefit and accuracy improvement of considering
multi-branch coupling when calculating EM induced stress, or further failure
prediction.
